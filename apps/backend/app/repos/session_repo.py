"""
Campaign Session repository — hybrid JSON/DB backend.
"""
...
SESSIONS_FILE = DATA_DIR / "campaign_sessions.json"


class CampaignSessionRepo(HybridRepo):
    table_model = CampaignSession
    pk_field = "session_id"
    json_file = SESSIONS_FILE
    ...
    def get_by_request_id(cls, request_id: str) -> Optional[dict]:
        if USE_DB:
            ...
            with SessionLocal() as s:
                row = s.query(CampaignSession).filter(
                    CampaignSession.request_id == request_id
                ).first()
                return cls._row_to_dict(row) if row else None
        # JSON fallback
        data = _json_load(cls.json_file)
        for sid, sess in data.items():
            if sess.get("request_id") == request_id:
